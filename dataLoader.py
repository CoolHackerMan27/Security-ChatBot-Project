...
def data_generator(input_file, target_file, batch_size, max_length):
    ...
    # dataset = dataset.take(50).cache().repeat()#This only works if the dataset fits in memory, so small datasets or large memory(like a lot. 128GB+) Using take(K) and repeat() to force the dataset to be fully cached.
    dataset = dataset.take(50).cache()  # Use this if you have less memory
